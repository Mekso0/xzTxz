...
# ID группы для отправки сообщений о работе бота
GROUP_ID = -1002457050145  # Замените на ID вашей группы (ОБНОВЛЕНО)

# Функция для отправки сообщения в группу
def send_heartbeat():
    while True:
        try:
            bot.send_message(GROUP_ID, "Я работаю! Не переживайте!")
            logging.info(f"Отправлено сообщение о работе бота в группу {GROUP_ID}")
        except Exception as e:
            logging.error(f"Ошибка при отправке сообщения о работе бота: {e}")
...
# Запускаем поток для отправки сообщений о работе бота
heartbeat_thread = threading.Thread(target=send_heartbeat)
heartbeat_thread.daemon = True  # Завершить поток, когда основная программа завершится
